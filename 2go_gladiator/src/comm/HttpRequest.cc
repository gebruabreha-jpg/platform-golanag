...
HttpRequest::HttpRequest() { output = nullptr; }
...
HttpRequest::HttpRequest(const HttpRequest &other) {
  ...
  // deep copy
  if (other.data_len > 0) {
    memcpy(data_body, other.data_body,
           std::min(MAX_HTTP3_BODY_DATA_LEN, other.data_len));
    data_len = other.data_len;
  }
}

bool HttpRequest::parse_uri(const char *uri) {
  http_parser_url u;
